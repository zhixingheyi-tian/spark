...
class StorageLevel private(
  ...
  private def this(flags: Int, replication: Int) {
    this((flags & 16) != 0, (flags & 8) != 0, (flags & 4) != 0, (flags & 2) != 0, (flags & 1) != 0, replication)
  }
  ...
  override def clone(): StorageLevel = {
    new StorageLevel(useDisk, useMemory, useOffHeap, useAEP, deserialized, replication)
  }
  ...
  def isValid: Boolean = (useMemory || useDisk || useAEP) && (replication > 0)
  ...
  override def toString: String = {
    ...
    val aep = if (useAEP) "aep" else ""
    val deserialize = if (deserialized) "deserialized" else ""

    val output =
      Seq(disk, memory, heap, aep, deserialize, s"$replication replicas").filter(_.nonEmpty)
...
object StorageLevel {
  ...
  val AEP = new StorageLevel(true, true, false, true, false, 1)
  ...
  def apply(
    ...
    getCachedStorageLevel(new StorageLevel(useDisk, useMemory, false, false, deserialized, replication))
  }
