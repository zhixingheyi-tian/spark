...
private[spark] class MemoryStore(
  ...
  // Note: aep unroll memory is only used in putIteratorAsBytes() because aep caching
  // always stores serialized values.
  private val aepUnrollMemoryMap = mutable.HashMap[Long, Long]()
  ...
  private def maxMemory: Long = {
    memoryManager.maxOnHeapStorageMemory +
      ...
      memoryManager.maxAEPStorageMemory
  }
    ...
    memoryManager.synchronized {
      ...
      if (success) {
        ...
        val unrollMemoryMap = memoryMode match {
          ...
          case MemoryMode.AEP => aepUnrollMemoryMap
        }
  ...
  def currentUnrollMemory: Long = memoryManager.synchronized {
    onHeapUnrollMemoryMap.values.sum + offHeapUnrollMemoryMap.values.sum + aepUnrollMemoryMap.values.sum
  }
  ...
  def currentUnrollMemoryForThisTask: Long = memoryManager.synchronized {
    onHeapUnrollMemoryMap.getOrElse(currentTaskAttemptId(), 0L) +
      ...
      aepUnrollMemoryMap.getOrElse(currentTaskAttemptId(), 0L)
  }
  ...
  private def numTasksUnrolling: Int = memoryManager.synchronized {
    (onHeapUnrollMemoryMap.keys ++ offHeapUnrollMemoryMap.keys ++ aepUnrollMemoryMap.keys).toSet.size
  }
